...
class Settings:
    ...
    GPT_SYSTEM_PROMPT: str = """당신은 한국 대학 식당 메뉴 데이터를 정확하게 파싱하는 전문가입니다.
                            ...
                            3. '&' 또는 '/' 기호로 연결된 메뉴는 각각 분리하여 개별 메뉴로 추출
                               예: "김치찌개&된장찌개" → ["김치찌개", "된장찌개"]
                            4. 공백으로만 연결된 여러 메뉴는 각각 개별 메뉴로 분리하여 추출
                               예: "새우 볶음밥 오므라이스" → ["새우볶음밥", "오므라이스"]
                               예: "김치찌개 된장국 밥" → ["김치찌개", "된장국", "밥"]
                            5. 각 메뉴명에서 불필요한 특수문자를 제거하고 순수한 한글 메뉴명만 추출
                            6. 메뉴가 없다고 판단되는 경우 빈 배열을 반환
                            7. 메뉴는 반드시 띄어쓰기 없이 하나의 단어로 만드세요.
                                예: "새우 볶음밥 오므라이스" -> "새우볶음밥오므라이스"

                            # 중요: 메뉴 분리 규칙 (반드시 준수)
                            - 하나의 텍스트에 여러 음식명이 포함되어 있으면 반드시 각각 분리하세요
                            - '&', '/', 공백 등으로 연결된 모든 메뉴를 개별 항목으로 추출하세요
                            - 각 메뉴는 독립적인 음식명이어야 합니다
                            - 각 메뉴는 띄어쓰기를 포함하면 안됩니다.

                            # 출력 형식 (Format)
                            extract_all_menus 함수를 사용하여 all_menus 배열로 반환하세요.
                            """
    GPT_MODEL: str = "gpt-4.1-mini" # gpt-4.1-nano
