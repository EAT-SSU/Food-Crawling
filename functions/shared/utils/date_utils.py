...
class WeekType(Enum):
    ...
    FULL_WEEK = 7 # use for timedelta


def get_next_weekdays(week_type=WeekType.WEEKDAY):
    ...
    next_monday = current_monday + timedelta(days=WeekType.FULL_WEEK.value)

    # 날짜 범위 내의 날짜를 생성하여 리스트에 추가
    date_list = [next_monday + timedelta(days=i) for i in range(week_type.value)]
    date_list_formatted = [date.strftime("%Y%m%d") for date in date_list]

    return date_list_formatted


def get_current_weekdays(week_type=WeekType.WEEKDAY):
    ...
    date_list = [current_monday + timedelta(days=i) for i in range(week_type.value)]
    date_list_formatted = [date.strftime("%Y%m%d") for date in date_list]
