...
class DodamScraper(MenuScraperInterface):
    ...
    async def scrape_menu(self, date: str) -> RawMenuData:
        ...
        try:
            ...
            raise record_fetch_failure(error, date, RestaurantType.DODAM) from None
    ...
    def _check_for_holidays(self, html_content: str, date: str) -> None:
        if is_explicit_day_closure(html_content):
            ...
            emit_source_result(result, date, RestaurantType.DODAM)
            raise HolidayException(date, RestaurantType.DODAM, html_content)

    def _parse_menu_from_html(self, html_content: str, date: str) -> RawMenuData:
        return classify_general_source(html_content, date, RestaurantType.DODAM)
