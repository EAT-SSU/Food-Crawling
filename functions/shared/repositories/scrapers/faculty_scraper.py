...
class FacultyScraper(MenuScraperInterface):
    """교직원식당 웹 스크래퍼"""
    ...
    def __init__(self, settings=None):
        ...
        self.restaurant_code = settings.SOONGGURI_FACULTY_RCD

    async def scrape_menu(self, date: str) -> RawMenuData:
        """교직원식당 메뉴를 스크래핑합니다."""
        logger.info(f"교직원식당 메뉴 스크래핑 시작: {date}")
        ...
        logger.info(f"교직원식당 메뉴 스크래핑 완료: {date}")
    ...
    def _check_for_holidays(self, html_content: str, date: str) -> None:
        ...
        if soup.find(text="오늘은 쉽니다.") or "휴무" in soup.text:
            raise HolidayException(target_date=date, restaurant_type=RestaurantType.FACULTY, raw_data=html_content)
    ...
    def _parse_menu_from_html(self, html_content: str, date: str) -> RawMenuData:
        ...
        if not raw_menu_data.menu_texts:
            raise MenuFetchException(target_date=date, restaurant_type=RestaurantType.FACULTY,
                                     raw_menu_data=raw_menu_data)
